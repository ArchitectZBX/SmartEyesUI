...
//NSMutableSet *cameras = [[NSMutableSet alloc] init];
UsePPPPChannelManagement *channelManage = [[UsePPPPChannelManagement alloc] init];
...
int CSearchDVS::Open()
{
    ...
    for(i=0;i<MAX_BIND_TIME;i++)
    {
    	...
    	iRet = bind(m_nSocket, (sockaddr *)&servaddr, sizeof(servaddr));    //将 socket 与特定主机地址与端口号绑定，成功绑定返回0，失败返回 -1。
    	if (iRet < 0)
    	{		
    		continue ;
    	}
...
void CSearchDVS::ReceiveProcess()
{
    ...
    {  
        ...
        /*
         ...
         recvfrom(int sockfd,void *buf,int len,unsigned int flags, struct sockaddr *from,socket_t *fromlen)
        ...
        {
            m_bRecvThreadRuning = false;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"sendDevicesNotification" object:cameras userInfo:nil];
        }
        else
        {
...
BOOL CSearchDVS::checkCamera(NSString *did) {
...
void CSearchDVS::GetNetParam(PBCASTPARAM pstParam)
{
    ...
    //想camera数组中添加摄像机（注意要去重）
    NSString *did = [NSString stringWithUTF8String:pstParam->dwDeviceID];
    NSString *name = [NSString stringWithUTF8String:pstParam->szDevName];
    [channelManage connectDevice:did password:@"888888"];
    NSDictionary *cameraDic = [NSDictionary dictionaryWithObjectsAndKeys:name,@STR_NAME,did, @STR_DID,nil];
    if (isFirst) {
...
void CSearchDVS::addCameraToDictionary() {
