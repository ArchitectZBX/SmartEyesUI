...
class YYSegmentedControl: UIControl {
    ...
    var _segmentIndicatorInset:CGFloat?
    var segmentIndicatorInset:CGFloat?{
    set{
    ...
    var segmentIndicatorBorderWidth:CGFloat?{
    set{
    ...
    var segmentIndicatorCornerRadius:CGFloat?{
    set{
    ...
    init(items: NSArray){
        ...
        self.initialize()
        let mutableSegments = NSMutableArray()
        
        for segmentTitle : AnyObject in items {
            let segment = YYSegment(title: segmentTitle as! NSString)
    ...
    override func sizeThatFits(size: CGSize) -> CGSize {
        ...
        var maxSegmentWidth:CGFloat = 0.0
        
        for  segment:AnyObject in self.segments! {
            let segmentWidth = segment.sizeThatFits(size).width
            if (segmentWidth > maxSegmentWidth) {
                maxSegmentWidth = segmentWidth
            }
        }
        let num = self.segments!.count
        let width:CGFloat = maxSegmentWidth * CGFloat(num)
        return CGSizeMake(width, 30.0)
    }

    override func layoutSubviews() {
        super.layoutSubviews()
        let segmentWidth = CGRectGetWidth(self.frame) / CGFloat(self.segments!.count)
        let segmentHeight = CGRectGetHeight(self.frame)
        for var i = 0 ;i < self.segments!.count;i++ {
            let segment = self.segments![i] as! YYSegment
            segment.frame = CGRectMake(segmentWidth * CGFloat(i), 0.0, segmentWidth, segmentHeight)
            
            
            if self.stylesTitleForSelectedSegment != nil && self.selectedSegmentIndex == i {
    ...
    {
        if (self.drawsGradientBackground != nil) {
    ...
    func insertSegmentWithTitle(title title:NSString,var index:Int){
        let num = self.segments!.count
        if index > num {
            index = self.segments!.count
        }
        
        let newSegment:YYSegment = YYSegment(title:title)
        self.addSubview(newSegment)
        
        let mutableSegments = NSMutableArray(array:self.segments!)
    ...
    func removeSegmentAtIndex(var index:Int){
        ...
        let segment:YYSegment = self.segments![index] as! YYSegment
    ...
    func setTitle(title:NSString,index:Int){
        let segment = self.segments![index] as! YYSegment
        segment.titleLabel!.text = title as String
    }
    ...
    func setSelectedSegmentIndex(index index:Int,animation:Bool){
